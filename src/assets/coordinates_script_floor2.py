...
with open('coordinates_floor2_public.txt', 'r') as file:
    content = file.read()
...
def parse_circle_data(data):
    ...
    for match in matches:
        id = match[0]
        circle_data = {
            "x": int(round(float(match[1])*0.285)) + 285,
            "y": int(round(float(match[2])*0.285)) + 200,
...
with open('mites_coordinates_floor2_updated.json', 'w') as json_file:
